...
if [ ${#gpu_ids} -gt 1 ]; then
    ...
    NGPU=4
    HOST_NODE_ADDR="localhost:29500"
    echo "HERE"
fi
################
...
vq_ckpt="/home/data/liangzhichao/Code/octfusion-main/logs/skeleton6_union/test_snet_lr1e-3/ckpt/vae_steps-latest.pth"
...
if [ $mode = "train" ]; then
    pretrain_ckpt="/home/data/liangzhichao/Code/octfusion-main/logs/skeleton6_union/union_2t_test_lr2e-4/ckpt/df_steps-latest.pth"
else
    ckpt="/home/data/liangzhichao/Code/octfusion-main/logs/skeleton6_rifle_union/union_2t_test_lr2e-4/ckpt/df_steps-latest.pth"
fi

if [ $mode = "generate" ]; then
    cond_dir="/mnt/gemlab_data_2/User_database/liangzhichao/QC_mask_mat"
else
    cond_dir="/mnt/gemlab_data_2/User_database/liangzhichao/simulated_mask_data_7"
fi
#cond_dir="/mnt/gemlab_data_2/User_database/liangzhichao/QC_mask_mat"
...
cmd="train.py --name ${name} --logs_dir ${logs_dir} --gpu_ids ${gpu_ids} --mode ${mode} \
    ...
    --debug ${debug} --cond ${cond} --cond_dir ${cond_dir}"
