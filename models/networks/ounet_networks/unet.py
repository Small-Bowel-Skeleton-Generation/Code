...
class OUNet(torch.nn.Module):
    ...
    def forward(self, octree_in, octree_out=None, update_octree: bool = False):
        ...
        # label_pre = out['signal']
        # octree_feature = ocnn.modules.InputFeature('P', nempty=True)
        # label_gt = octree_feature(octree_in)
        #
        # # 保存 label_gt 到 .mat 文件
        # save_path = '/home/data/liangzhichao/Code/octfusion-main/data/'
        # sio.savemat(os.path.join(save_path, 'label_gt.mat'), {'label_gt': label_gt.detach().cpu().numpy()})
