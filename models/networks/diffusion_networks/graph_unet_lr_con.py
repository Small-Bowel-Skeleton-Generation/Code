...
class UNet3DModel(nn.Module):
    ...
    def __init__(
        ...
        # self.input_emb = conv_nd(dims, 2 * self.in_channels, self.model_channels // 2, 3, padding=1)
            ...
            self.downs.append(nn.ModuleList([
                ...
                nn.Sequential(
                    ...
                    AttentionBlock(dim_out, num_heads=num_heads)) if not use_spatial_transformer else SpatialTransformer(
                        dim_out, num_heads, context_dim=context_dim),
                ConvDownsample(
        ...
        self.mid_self_attn = nn.Sequential(
            ...
            mid_dim, num_heads, context_dim=context_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                ...
                nn.Sequential(
                    ...
                    dim_in, num_heads, context_dim=context_dim),
                ConvUpsample(
    ...
    def forward(self, x=None, timesteps=None, x_self_cond=None, label=None, context=None, as_middle=False, **kwargs):
        ...
        for resnet, self_attn, downsample in self.downs:
            x = resnet(x, emb)
            x = self_attn(x, context)
            h.append(x)
            x = downsample(x)

        x = self.mid_block1(x, emb)
        x = self.mid_self_attn(x, context)
