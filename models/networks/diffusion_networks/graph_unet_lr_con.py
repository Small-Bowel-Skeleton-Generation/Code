...
from models.networks.mask_encoder import MultiCNN
...
class UNet3DModel(nn.Module):
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                nn.Sequential(
                    ...
                    activation_function(),
                    AttentionBlock(dim_out, num_heads=num_heads)) if not use_spatial_transformer else SpatialTransformer(
                        dim_out, num_heads, context_dim=self.context_dim),
                ConvDownsample(
        ...
        self.mid_self_attn = nn.Sequential(
            ...
            activation_function(),
            AttentionBlock(mid_dim, num_heads=num_heads)) if not use_spatial_transformer else SpatialTransformer(
            mid_dim, num_heads, context_dim=self.context_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(nn.ModuleList([
                ...
                nn.Sequential(
                    ...
                    activation_function(),
                    AttentionBlock(dim_in, num_heads=num_heads)) if not use_spatial_transformer else SpatialTransformer(
                    dim_in, num_heads, context_dim=self.context_dim),
                ConvUpsample(
    ...
    def forward_as_middle(self, h, doctree, timesteps, label, context, graph_octree):
        h_lr = octree2voxel(data=h, octree=doctree.octree, depth=self.full_depth)
        h_lr = h_lr.permute(0, 4, 1, 2, 3).contiguous()
        h_lr = self.forward(x=h_lr, timesteps=timesteps, label=label, context=context, graph_octree=graph_octree, as_middle=True)
    ...
    def forward(self, x=None, timesteps=None, x_self_cond=None, label=None, context=None, graph_octree=None, as_middle=False, **kwargs):
        """
