    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.in_planes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1x1(self.in_planes, planes * block.expansion, (stride, stride, stride)),
...
def resnet3D():
    model = ResNet3D(BasicBlock, [3, 3, 3, 3, 3], [8, 16, 32, 64, 128, 256])
    return model
...
class MultiCNN(nn.Module):
    def __init__(self, down_ratio, last_level, out_channel=0):
