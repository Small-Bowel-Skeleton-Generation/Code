...
class GraphConv(torch.nn.Module):
	...
	def forward(self, x, doctree, d):
		...
		# x -> col_data
		row, col = edge_index[0], edge_index[1]		# row：边的起点, col：边的终点
...
class GraphDownsample(torch.nn.Module):
	...
	def forward(self, x, doctree, d):    # 这里的写法是，GraphDownsample是把深度为d的对偶图下采样到深度为d-1
		...
		numd = doctree.nnum[d]		# 当前层（深度 d）的节点数量
		lnumd = doctree.lnum[d-1]		# 父节点（深度 d-1）的节点数量
		leaf_mask = doctree.node_child(d-1) < 0		# 深度 d-1 的哪些节点是叶子节点
		outd = x[-numd:]		# 提取深度 d 的特征
		outd = self.downsample(outd)	# 对深度 d 的节点进行下采样

		# get the nodes at layer (depth-1)
		out = torch.zeros(leaf_mask.shape[0], x.shape[1], device=x.device)
		out[leaf_mask] = x[-lnumd-numd:-numd]		# 深度 d-1 的叶子节点特征直接取输入
		out[leaf_mask.logical_not()] = outd		# 非叶子节点使用降采样后的子节点特征
...
class GraphUpsample(torch.nn.Module):
	...
	def forward(self, x, doctree, d):     # 这里的写法是，GraphUpsample是把深度为d的对偶图上采样到深度为d+1
		numd = doctree.nnum[d]		# 当前层节点数量
		leaf_mask = doctree.node_child(d) < 0		# 标识叶子节点
		# upsample nodes at layer (depth-1)
		outd = x[-numd:]		# 提取当前层的节点特征
		out1 = outd[leaf_mask.logical_not()]		# 非叶子节点特征
		out1 = self.upsample(out1)		# 对非叶子节点特征进行上采样
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head=4,
