...
# rendering
from utils.util_dualoctree import calc_sdf, octree2split_small, octree2split_large, split2octree_small, \
    split2octree_large
...
class OctFusionModel(BaseModel):
    ...
    def set_input(self, input=None):
        ...
        #                    save_dir='/home/data/liangzhichao/Code/octfusion-main/logs/skeleton5_union/union_2t_test_lr2e-4/results_skeleton5/latent',
        #                    index=1)
    ...
    def calc_loss(self, input_data, doctree_in, batch_id, unet_type, unet_lr, df_type="x0"):
        ...
        # scipy.io.savemat(input_data_path, {'input_data': input_data.detach().cpu().numpy()})
        # scipy.io.savemat(noised_data_path, {'noised_data': noised_data.detach().cpu().numpy()})
        # scipy.io.savemat(output_path, {'output': output.detach().cpu().numpy()})
    ...
    def forward(self):
        ...
        if self.stage_flag == "lr":
            ...
            self.df_hr_loss = self.calc_loss(self.input_data, self.doctree_in,
                                             self.doctree_in.batch_id(self.small_depth), "hr", self.df_module.unet_lr,
                                             self.df_type[1])
    ...
    @torch.no_grad()
    def sample_loop(self, doctree_lr=None, ema=False, shape=None, ddim_steps=200, label=None, cond=False,
                    cond_dir=None, unet_type="lr", unet_lr=None, df_type="x0", truncated_index=0.0):
        ...
        for t, t_next in time_iter:
            ...
            if ema:
                output = self.ema_df(unet_type=unet_type, x=noised_data, doctree=doctree_lr, timesteps=noise_cond,
                                     unet_lr=unet_lr, x_self_cond=x_start, label=label, context=context_pairs)
            else:
                output = self.df(unet_type=unet_type, x=noised_data, doctree=doctree_lr, timesteps=noise_cond,
                                 unet_lr=unet_lr, x_self_cond=x_start, label=label, context=context_pairs)
    ...
    @torch.no_grad()
    def sample(self, split_small=None, category='airplane', prefix='results', ema=False, ddim_steps=200, clean=False,
               save_index=0, cond=False, cond_dir=None):
        ...
        if split_small == None:
            ...
            split_small = self.sample_loop(doctree_lr=None, ema=ema, shape=(batch_size, *self.z_shape),
                                           ddim_steps=ddim_steps, label=label, cond=cond,
                                                 cond_dir=cond_dir, unet_type="lr", unet_lr=None,
                                           df_type=self.df_type[0], truncated_index=TRUNCATED_TIME)

        # # Save input_data and output to .mat files
        # save_dir = "/home/data/liangzhichao/Code/octfusion-main/logs/skeleton5_union/union_2t_test_lr2e-4/results_skeleton5/latent/"
        # os.makedirs(save_dir, exist_ok=True)
        # input_data_path = os.path.join(save_dir, f"split_small.mat")
        # scipy.io.savemat(input_data_path, {'split_small': split_small.detach().cpu().numpy()})

        # noised_data = torch.randn((batch_size, *self.z_shape), device=self.device)
        # noised_small = split2octree_small(noised_data, self.octree_depth, self.full_depth)
        # self.export_octree(noised_small, depth=self.small_depth, save_dir=os.path.join(save_dir, "latent"),
        #                    index=save_index)

        octree_small = split2octree_small(split_small, self.octree_depth, self.full_depth)
        self.export_octree(octree_small, depth=self.small_depth, save_dir=os.path.join(save_dir, "octree"),
                           index=save_index)
        ...
        samples = self.sample_loop(doctree_lr=doctree_small, shape=(doctree_small_num, self.code_channel), ema=ema,
                                   ddim_steps=ddim_steps, label=label, unet_type="hr", unet_lr=self.ema_df.unet_lr,
                                   df_type=self.df_type[1])
    ...
    def export_octree(self, octree, depth, save_dir=None, index=0):
        ...
        for i in range(batch_size):
            ...
            scipy.io.savemat(mat_path, {'data': voxel})
    ...
    def export_mesh(self, save_dir, index=0, level=0, clean=False):
        ...
        for i in range(ngen):
            ...
            if vtx.size == 0 or faces.size == 0:
                ...
                return
            vtx = vtx * ((
                                     self.bbmax - self.bbmin) / size) + self.bbmin  # [0,sz]->[bbmin,bbmax]  把vertex放缩到[bbmin, bbmax]之间
            vtx = vtx * mesh_scale
