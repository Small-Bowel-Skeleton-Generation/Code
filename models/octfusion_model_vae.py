...
class OctFusionModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.ckpt is None and os.path.exists(os.path.join(opt.logs_dir, opt.name, "ckpt/vae_steps-latest.pth")):
            opt.ckpt = os.path.join(opt.logs_dir, opt.name, "ckpt/vae_steps-latest.pth")
    ...
    def batch_to_cuda(self, batch):
        ...
        if self.load_pointcloud:
            ...
            for pts in points:
                ...
                sampled_points = point_tensor[indices]
                skeletons.append(sampled_points)
            batch['skeleton_gt'] = torch.stack(skeletons)  # 形状为 (batch_size, 512, 3)
    ...
    def inference(self, save_folder = "results_vae"):
        ...
        # self.export_pointcloud(save_dir, output['octree_out'], depth=8)
        signal = output['signal'].cpu().numpy()  # Ensure it's on CPU and convert to numpy array
        signal_mat_path = os.path.join(save_dir, 'signal.mat')
        scipy.io.savemat(signal_mat_path, {'signal': signal})

        # signal_noise = output['noise_skel'].cpu().numpy()  # Ensure it's on CPU and convert to numpy array
        # noise_mat_path = os.path.join(save_dir, 'signal_noise.mat')
        # scipy.io.savemat(noise_mat_path, {'signal_noise': signal_noise})
        #
        # skeleton = output['ounet_out']['signal'].cpu().numpy()
        # skeleton_mat_path = os.path.join(save_dir, 'skeleton.mat')
        # scipy.io.savemat(skeleton_mat_path, {'skeleton': skeleton})
        ...
        pointcloud = trimesh.PointCloud(vertices=self.batch['curve'][0]['points'].cpu().points.numpy())
        pointcloud.export(os.path.join(save_dir, 'input.ply'))

        sdf_mat_path = os.path.join(save_dir, 'sdfs.mat')
        scipy.io.savemat(sdf_mat_path, {'sdfs': self.sdfs.cpu().numpy()})
    ...
    def export_pointcloud(self, save_dir, octree, index=0, depth=None):
        """
        Extract point cloud from an octree and save it to a PLY file.

        Args:
            save_dir (str): Directory to save the point cloud.
            octree: Octree object containing the point cloud data.
            index (int): Index of the point cloud for naming the output file.
            depth (int): Specific depth level to extract points from (optional).
                         If None, extract points from all leaf nodes.
        """
        try:
            os.makedirs(save_dir, exist_ok=True)
        except FileExistsError:
            pass
        points_list = []
        # Extract points from specific depth or all depths
        depth_range = range(octree.full_depth, octree.depth + 1) if depth is None else [depth]
        for d in depth_range:
            if octree.nnum[d] > 0:  # Ensure there are nodes at this depth
                keys = octree.keys[d].cpu().numpy()  # Get keys as numpy array
                size = 2 ** d  # Number of divisions in the grid at depth `d`

                # Decode keys into point coordinates (x, y, z)
                x = (keys >> 0) & (size - 1)
                y = (keys >> d) & (size - 1)
                z = (keys >> (2 * d)) & (size - 1)

                points = np.stack([x, y, z], axis=1).astype(np.float32)
                points = points / size * 2 - 1  # Scale to [-1, 1]
                points_list.append(points)

                # Debugging output
                print(f"Depth {d}: Extracted {len(points)} points")
                print(f"x: min={x.min()}, max={x.max()}")
                print(f"y: min={y.min()}, max={y.max()}")
                print(f"z: min={z.min()}, max={z.max()}")

        # Combine points from all depths
